...
local eggTake = ReplicatedStorage:WaitForChild("Shared"):WaitForChild("Remotes"):WaitForChild("EggTake")

local autoClickEnabled = false
local clickRange, clickDelay = 100, 0.15
local lastClick = 0
local autoEggEnabled, useEggRemote, eggDelay, lastEgg = false, false, 0.75, 0
local eggIds, eggScanRunning = {}, false
local autoSkillQ, autoSkillE, autoSkillR, skillDelay, lastSkill = false, false, false, 1, 0
		...
		for index, data in ipairs(getgc(true)) do
			if type(data) == "table" then
				for key, value in pairs(data) do
					if type(key) == "number" and eggs[value] and not eggIds[value] then
						eggIds[value], mapped = key, mapped + 1
						print("[Egg ID]", key, "=", value.Name)
					end
					if eggs[key] and type(value) == "number" and not eggIds[key] then
						eggIds[key], mapped = value, mapped + 1
						print("[Egg ID]", value, "=", key.Name)
					end
				end
			end
...
local function resetAll()
	autoClickEnabled, autoEggEnabled, useEggRemote, autoSkillQ, autoSkillE, autoSkillR, speedEnabled, flyEnabled, jumpEnabled, antiAfkEnabled = false, false, false, false, false, false, false, false, false, false
...
task.spawn(function()
	while true do
		if os.clock() - lastSkill >= skillDelay and (autoSkillQ or autoSkillE or autoSkillR) then
			if autoSkillQ then useSkill(Enum.KeyCode.Q) end
			if autoSkillE then useSkill(Enum.KeyCode.E) end
			if autoSkillR then useSkill(Enum.KeyCode.R) end
			lastSkill = os.clock()
		end
		task.wait(0.05)
	end
...
CombatTab:Divider()
CombatTab:Toggle({ Title = "Auto Skill Q", Default = false, Callback = function(value) autoSkillQ = value end })
CombatTab:Toggle({ Title = "Auto Skill E", Default = false, Callback = function(value) autoSkillE = value end })
CombatTab:Toggle({ Title = "Auto Skill R", Default = false, Callback = function(value) autoSkillR = value end })
CombatTab:Slider({ Title = "Skill Delay (seconds)", Step = 0.1, Value = { Min = 0.1, Max = 10, Default = 1 }, Callback = function(value) skillDelay = value end })
CombatTab:Divider()
...
FarmTab:Button({ Title = "Scan Egg IDs", Icon = "search", Callback = scanEggIds })
FarmTab:Toggle({ Title = "Use EggTake ID", Desc = "เปิดหลัง Scan พบ ID ที่ถูกต้อง", Default = false, Callback = function(value) useEggRemote = value end })
